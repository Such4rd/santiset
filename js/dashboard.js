...
function metricRows(entries, variant=''){
  return entries.map(([label,value]) => `<div class="metric-row"><span class="metric-label">${label}</span><span class="metric-value ${variant}">${value}</span></div>`).join('');
}
...
function renderWonLost(points){
  const wonRivalErrors = [
    ...
    ['Doble falta', count(points, p => p.outcome === OUTCOMES.RIVAL_UNFORCED_ERROR && /doble/i.test(`${p.stroke_type || ''} ${p.cause_key || ''} ${p.notes || ''}`))],
    ['Resto', count(points, p => p.outcome === OUTCOMES.RIVAL_UNFORCED_ERROR && p.stroke_type === 'resto')]
  ];
...
function serveCard(title, data){
  return `<article class="card serve-card"><h4>${title}</h4><div class="serve-numbers"><div class="serve-mini"><span>Jugados</span><strong>${data.played}</strong></div><div class="serve-mini"><span>Ganados</span><strong>${data.won}</strong></div><div class="serve-mini"><span>Perdidos</span><strong>${data.lost}</strong></div></div><div class="serve-pct">${data.pct}%</div></article>`;
}
function renderServeReturn(points){
  $('#serveStats').innerHTML = [serveCard('1º saque', serveBlock(points,'1º','SERVE')), serveCard('2º saque', serveBlock(points,'2º','SERVE'))].join('');
  $('#returnStats').innerHTML = [serveCard('1º saque', serveBlock(points,'1º','RETURN')), serveCard('2º saque', serveBlock(points,'2º','RETURN'))].join('');
}
