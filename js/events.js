...
function youtubeMoment(url, seconds){ if(!url) return ''; const sep=url.includes('?')?'&':'?'; return `${url}${sep}t=${Math.max(0,Number(seconds||0)-7)}s`; }
...
function render(){
  ...
  $('#eventsTable').innerHTML=pts.length?`<div class="event-list">${pts.map((p,i)=>{ const s=db.sets.find(x=>x.id===p.set_id); const video=s?.video_url || match?.video_url || ''; const link=youtubeMoment(video,p.video_second||p.point_duration_seconds); const won=p.point_result==='WON'; return `<article class="event-row-card ${won?'won':'lost'}"><div class="event-main"><div class="event-topline"><strong>S${s?.set_number||'-'} · P${p.point_id||i+1}</strong><span class="result-chip ${won?'won':'lost'}">${won?'Ganado':'Perdido'}</span>${+p.highlight===1?'<span class="star-chip">★</span>':''}</div><div class="event-score-line"><span class="score-after">Juego: <b>${p.our_points_after||'?'} - ${p.rival_points_after||'?'}</b></span><span class="score-after">Set: <b>${p.our_games_after??'?'} - ${p.rival_games_after??'?'}</b></span></div><div class="event-stroke"><b>${p.player_id||'Rival'}</b> · ${shortCat(p.stroke_category)} · ${p.stroke_type||p.court_zone||'-'}</div><div class="event-finish">${outcomeText(p.outcome)} · ${p.rally||'-'} · ${p.server||'-'} ${p.serve_number||''}</div></div><div class="event-video">${link?`<a class="youtube-icon-link" target="_blank" href="${link}" title="Abrir momento en YouTube">${youtubeIcon()}</a>`:'-'}</div></article>`; }).join('')}</div>`:'<p class="small">No hay eventos con esos filtros.</p>';
}
